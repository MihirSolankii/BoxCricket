  ...
  const fetchTurfData = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/admin/turf",
        {
          headers: {
      ...
      // Assuming response format: { data: [...], turfId: "..." }
      const { data, turfId } = response.data;

      // Store in localStorage
      localStorage.setItem('boxDetails', JSON.stringify(data[0] || {})); // store first turf
      localStorage.setItem('boxId', turfId);

      console.log("Turf data loaded:", data, "TurfId:", turfId);
    } catch (error) {
  ...
  fetchTurfData();
    ...
    const handleUpdateGeneral = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(
                `http://localhost:5000/api/admin/update-box-details/${boxId}`,
                payload,
                getAuthHeader()
            );
    ...
    const handleUpdateCourt = async (courtId, isAvailable) => {
        ...
        try {
            ...
            const response = await axios.put(
                `http://localhost:5000/api/admin/update-court-availability/${boxId}`,
                payload,
                getAuthHeader()
            );
    ...
    const handleUpdateHours = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(
                `http://localhost:5000/api/admin/update-operating-hours/${boxId}`,
                payload,
                getAuthHeader()
            );
    ...
    const handleDeleteBox = async () => {
        ...
        try {
            ...
            const response = await axios.delete(
                `http://localhost:5000/api/admin/delete-box/${boxId}`,
                getAuthHeader()
            );
